initKG = function (data, config, container) {
    ...
    links.forEach(function (link) {
        ...
        link.source = nodeDict[link.from_node]
        console.log(link.from_node, link.source)
        nodes[link.source.name] = link.source

        link.target = nodeDict[link.to_node]
        nodes[link.target.name] = link.target

        link.rela = link.score.toFixed(3)
        console.log(link) 
    ...
    Object.keys(data.nodes).forEach(function (key) {
        ...
        if (colorDict[type] == null) {
            ...
            point = (point + 2) % defaultNodeColor.length
        }
